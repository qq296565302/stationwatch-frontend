...
#
# 【功能说明】
# 生成 VBScript 脚本，用于检查并锁定/解锁过期的值班工作表
#
# 【使用方法】
# 运行此脚本后，会生成 check_lock.vbs 文件
# 双击运行 check_lock.vbs 即可检查所有工作表的锁定状态
#
# 【工作原理】
# 1. 查找当月的值班记录文件（值班记录_YYYY-MM.xlsx）
# 2. 检查所有日期格式的工作表（YYYY-MM-DD）
# 3. 如果当前时间超过截止时间（Z1单元格），锁定工作表
# 4. 如果当前时间未到截止时间，解锁工作表
#
# 【配置项】
# 如需修改硬编码路径，请修改第52行
#
...
# 生成 VBScript 文件
vbs_path = r"c:\Users\m1316\Desktop\值班记录表\vbs\check_lock.vbs"
...
print()
print("使用方法：")
print("1. 双击运行 vbs\\check_lock.vbs")
print("2. 系统会自动检查并锁定过期的工作表")
print()
print("配置说明：")
print("- 锁定密码：DutyLock2026")
print("- 截止时间：存储在工作表的 Z1 单元格")
print("- 锁定规则：当前时间 > 截止时间，自动锁定")
print("============================================================")
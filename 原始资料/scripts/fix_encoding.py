...
1. 修改下方配置项（有详细注释标注）
2. 保存文件
3. 双击运行此文件（需要安装 Python）
4. 自动生成新的 auto_duty.vbs

注意：
- 此文件是 UTF-8 编码，可以正常编辑
- 生成的 auto_duty.vbs 是 GBK 编码，在 IDE 中会显示乱码（这是正常的）
- 修改时请保持引号、逗号等符号格式不变
...
【示例1】修改值班人员名单
原：dutyPersons = Array("张三、李四", "王五、赵六")
改：dutyPersons = Array("张三、李四、王五", "赵六、钱七")
说明：每人用"、"分隔，每组用英文逗号+空格分隔

【示例2】修改基准日期
原：baseDate = DateValue("2026-07-26")
改：baseDate = DateValue("2026-08-01")
说明：这是值班表开始的第一天，用于计算轮换周期

【示例3】修改截止时间
原：DEADLINE_TIME = "08:30:00"
改：DEADLINE_TIME = "09:00:00"
说明：格式为 "HH:MM:SS"（时:分:秒）

【示例4】修改供电所名称
原：STATION_NAME = "**供电中心**供电所"
改：STATION_NAME = "**XX供电中心**XX供电所"
说明：显示在第一行标题中，**部分不会被识别为加粗标记（仅作示例符号）**
"""

import os

# ============================================
# VBScript 脚本内容（请勿随意修改，除非了解 VBScript 语法）
# ============================================
vbs_content = '''\' ============================================
\' 值班记录表自动生成脚本
\' 版本：2.0
...
# Write VBS file with GBK encoding
vbs_path = r"c:\Users\m1316\Desktop\值班记录表\vbs\auto_duty.vbs"
...
print("配置生成成功！")
print("=" * 60)
print(f"生成文件：{vbs_path}")
print("\n后续步骤：")
print("1. 双击运行 auto_duty.vbs")
print("2. 检查生成的值班记录表是否正确")
print("\n功能说明：")
print("✓ 自动创建每日值班工作表")
print("✓ 自动填写值班人员和时间")
print("✓ 次日 8:30 自动锁定工作表")
print("✓ 锁定密码：DutyLock2026")
print("=" * 60)